...
export default function AuditLogsPage() {
  ...
  const createCount = logs.filter(l => getActionCategory(l.action) === 'Create').length
  const updateCount = logs.filter(l => getActionCategory(l.action) === 'Update').length
  const deleteCount = logs.filter(l => getActionCategory(l.action) === 'Delete').length
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={
          <Btn onClick={fetchLogs} icon="refresh" className="flex items-center gap-1.5">
            Refresh Logs
          </Btn>
        }
      />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Operations', val: totalLogs, color: 'text-indigo-600', bg: 'bg-indigo-50 border-indigo-100' },
          { label: 'Creations', val: createCount, color: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-100' },
          { label: 'Updates', val: updateCount, color: 'text-amber-600', bg: 'bg-amber-50 border-amber-100' },
          { label: 'Deletions', val: deleteCount, color: 'text-red-600', bg: 'bg-red-50 border-red-100' },
        ].map((stat, i) => (
          <div key={i} className={`p-4 rounded-2xl border bg-white/70 backdrop-blur-sm shadow-sm flex flex-col justify-between ${stat.bg}`}>
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{stat.label}</span>
            <span className={`text-2xl sm:text-3xl font-extrabold mt-2 ${stat.color}`}>{stat.val}</span>
          </div>
        ))}
      </div>
