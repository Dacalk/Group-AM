...
export function UserCredentials({ setPage, data, refreshData, setEditingUser, currentUser }) {
  ...
  const handleDelete = async (id, name) => {
    if (currentUser && String(id) === String(currentUser.id)) {
      await alert('You cannot delete your own account.', 'error', 'Action Forbidden');
      return;
    }
                  ...
                  <td className="py-3.5">
                    <div className="flex items-center gap-3 text-sm">
                      ...
                      {currentUser && String(currentUser.id) === String(u.id) ? (
                        <span className="text-gray-300 font-semibold cursor-not-allowed" title="You cannot delete your own account">Delete</span>
                      ) : (
          ...
          {filteredUsers.map((u) => (
            <div key={u.id} className="border border-gray-100 rounded-xl p-4 bg-gray-50/50">
              ...
              <div className="flex gap-3 border-t border-gray-100 pt-3">
                ...
                {currentUser && String(currentUser.id) === String(u.id) ? (
                  <span className="flex-1 text-center text-sm text-gray-300 font-semibold cursor-not-allowed" title="You cannot delete your own account">Delete</span>
                ) : (
...
export function AddUser({ setPage, refreshData, editingUser, setEditingUser }) {
  ...
  const [className, setClassName] = useState(editingUser?.className || '')
  const [gender, setGender] = useState(editingUser?.gender || 'Male')
  const [dob, setDob] = useState(editingUser?.dob || '')
  const [admissionNo, setAdmissionNo] = useState(editingUser?.admissionNo || '')
  const [medicalInfo, setMedicalInfo] = useState(editingUser?.medicalInfo || '')

  // Teacher-specific fields
  const [departments, setDepartments] = useState([])
  const [departmentId, setDepartmentId] = useState(editingUser?.departmentId || '')
  ...
  const [yearsExperience, setYearsExperience] = useState(editingUser?.yearsExperience || '')
  const [salary, setSalary] = useState(editingUser?.salary || '')

  // Parent-specific fields
  const [studentsList, setStudentsList] = useState([])
  const [studentId, setStudentId] = useState(editingUser?.studentId || '')
  const [relationship, setRelationship] = useState(editingUser?.relationship || 'Parent')
