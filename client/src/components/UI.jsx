...
export function Btn({ children, onClick, variant = 'primary', icon, className = '', ...props }) {
  const styles = {
  ...
  return (
    <button
      onClick={onClick}
      className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed ${styles[variant]} ${className}`}
      {...props}
    >
      {icon && (
        typeof icon === 'string' ? (
          <Icon path={icons[icon]} size={14} />
        ) : (
          icon
        )
      )}
