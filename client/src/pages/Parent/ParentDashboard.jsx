...
function MyChildrenPage({ onMenuClick, children }) {
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-5">
        {children.map((child) => (
          <section key={child.id} className="w-[260px] shrink-0 rounded-[14px] border border-gray-200 bg-white p-5">
            ...
            {[["Roll No", child.rollNo], ["Date of Birth", formatDOB(child.dob)], ["Gender", child.gender], ["Attendance", `${child.attendance}%`]].map(([key, value]) => (
        ...
        <table className="w-full border-collapse text-[13px]">
          <thead>
            <tr className="border-b border-gray-200 bg-[#f9fafb]">
              {["#", "CHILD NAME", "USERNAME", "CLASS", "EMAIL", "LAST LOGIN", "STATUS"].map((h) => (
          ...
          <tbody>
            {filtered.map((child, index) => (
              <tr key={child.id} className="border-b border-[#f0f0f0]">
                ...
                <td className="px-3 py-3 text-[#555]">{child.email}</td>
...
export default function ParentDashboard({ onRoleChange, currentUser }) {
  ...
  const renderPage = () => {
    ...
    switch (page) {
      ...
      case "children":
        return <MyChildrenPage {...pageProps} children={children} />;
      case "accounts":
